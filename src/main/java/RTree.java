...
public class RTree<TKey extends Comparable<TKey>, TValue> implements Serializable {
    ...
    public RTree(Table table, String colKey, int nodeSize) throws DBAppException {
        this.colKey = colKey;
        count = 0;
        this.nodeSize = nodeSize;
        this.bTreeName = table.name + "_" + this.colKey;
        this.nodes = new ArrayList<String>();
        this.root = new RTreeLeafNode<TKey,TValue>(this);
    ...
    public RTreeNode<TKey> loadNodeData(String nodeName) throws DBAppException {
        ...
        try {
            if(!nodeName.equals("")){
                ...
                RTreeNode<TKey> v = (RTreeNode<TKey>) in.readObject();
    ...
    public void saveNodeData(String nodeName, RTreeNode<TKey> v) throws DBAppException {
        try {
    ...
    public int insert(TKey key, TValue value) throws DBAppException {
        RTreeLeafNode<TKey, TValue> leaf = readBPTree(this.bTreeName).findLeafNodeShouldContainKey(key);
        ...
        System.out.println("++++++++++++++++++++++++++++++++");
        RTreeLeafNode<TKey, TValue> leafNode = (RTreeLeafNode<TKey, TValue>) loadNodeData(leaf.leftSibling);
        while(leafNode != null){
            System.out.println("++++++++++++++++++++++++++++++++");
            ...
            System.out.println("++++++++++++++++++++++++++++++++");
            leafNode = (RTreeLeafNode<TKey, TValue>) loadNodeData(leafNode.leftSibling);
        }
        if (leaf.isOverflow()) {
            RTreeNode<TKey> n = leaf.dealOverflow();
            if (n != null) {
                RTreeNode<TKey> node = loadNodeData((String) readBPTree(this.bTreeName).nodes.get(0));
    ...
    public LinkedList search(TKey key, myPolygon polygon , String operation) throws DBAppException {
        RTreeLeafNode<TKey, TValue> leaf = this.findLeafNodeShouldContainKey(key);
        LinkedList<Hashtable> ret = new LinkedList<Hashtable>();
        int index = leaf.search(key);
            ...
            if (index != -1) {
                if (operation.equals("=")) {
                    for (int i = index; i <= leaf.keyCount; i++) {
                        if (leaf.keys[i] != null && ((TKey) leaf.keys[i]).equals(key) && polygon.equalsto((myPolygon) ((Hashtable)leaf.getValue(i)).get(this.colKey))) {
                            ret.add((Hashtable) leaf.getValue(i));
                        } else {
                            if (leaf.keys[i] == null && i != leaf.keyCount) {
                                leaf = null;
                                if (leaf.rightSibling != null)
                                    leaf = (RTreeLeafNode<TKey, TValue>) loadNodeData(leaf.rightSibling);
                                break;
                            }
                        }
                    }
                    ...
                    if (operation.equals("!=")) {
                        RTreeNode<TKey> node = loadNodeData(this.nodes.get(0));
                        while(!node.parentNode.equals(""))
                        {
                        ...
                        while (node.getNodeType() == TreeNodeType.InnerNode) {
                            this.saveNodeData(node.nodeName,node);
                            node = ((RTreeInnerNode<TKey>) node).getChild();
                        }
                        while(node != null){
                            for(int i = 0 ; i < node.keyCount ; i++)
                            {
                                if(((TKey) leaf.keys[i]).equals(key) && polygon.equalsto((myPolygon) ((Hashtable)leaf.getValue(i)).get(this.colKey)) ){
                                    ret.add((Hashtable)((RTreeLeafNode<TKey, TValue>) node).getValue(i));
                                }
                            }
                            if(node != null){
                ...
                if (operation.equals("!=")) {
                    RTreeNode<TKey> nodeLeft = loadNodeData(this.nodes.get(0));
                    ...
                    while (nodeLeft.getNodeType() == TreeNodeType.InnerNode) {
                        nodeLeft = ((RTreeInnerNode<TKey>) nodeLeft).getChild("left");
                    }
                    while (nodeLeft != null) {
                        for (int i = 0; i < nodeLeft.keyCount; i++) {
                            if (nodeLeft.getKey(i) != null) {
                                ret.add((Hashtable) ((RTreeLeafNode) nodeLeft).getValue(i));
                            }
                        }
                    ...
                    if (operation.equals("<=") || operation.equals("<")) {
                        ...
                        if (operation.equals(">=") || operation.equals(">")) {
                            RTreeNode<TKey> nodeRight = loadNodeData(this.nodes.get(0));
                            ...
                            while (nodeRight.getNodeType() == TreeNodeType.InnerNode) {
                                nodeRight = ((RTreeInnerNode<TKey>) nodeRight).getChild("right");
                            }
                            while (nodeRight != null) {
                                for (int i = nodeRight.keyCount-1; i >= 0; i--) {
                                    if (nodeRight.getKey(i) != null && nodeRight.getKey(i).compareTo(key) > 0) {
                                        ret.addFirst((Hashtable) ((RTreeLeafNode) nodeRight).getValue(i));
                                    } else {
    ...
    public RTreeLeafNode<TKey, TValue> findLeafNodeShouldContainKey(TKey key) throws DBAppException {
        RTreeNode<TKey> node = loadNodeData(this.nodes.get(0));
        while(!node.parentNode.equals(""))
        {
            node = loadNodeData(node.parentNode);
